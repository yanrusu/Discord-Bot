...
from searchweather import get_status, status
from aqi import now_aqi
...
async def todayweather(ctx):
    await ctx.send(get_status())  # 發送天氣資訊至發送指令的頻道

@bot.command()
async def reportweather(ctx):
    jdata['weatherchannel'] = ctx.channel.id
    channel = bot.get_channel(ctx.channel.id)
    with open('TOKEN.json', 'w', encoding='utf8') as jsonf:
        json.dump(jdata, jsonf, ensure_ascii=False, indent=2)

    await channel.send(f"設定成功{jdata['weatherchannel']}")
...
async def getweather():
    ...
    if now == specified_time:  # 如果當前時間等於指定的時間，運行下方程式
        infweather = get_status()  # 獲取天氣資訊
        channel = bot.get_channel(jdata["weatherchannel"])  # 獲取指定的 Discord 頻道
        await channel.send(infweather)  # 發送天氣資訊至指定的頻道

@bot.command()
async def getaqi(ctx):
    await ctx.send(now_aqi())
